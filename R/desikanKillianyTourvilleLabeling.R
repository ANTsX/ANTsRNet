...
#' DeepAtropos.
#'
#' See \url{https://surfer.nmr.mgh.harvard.edu/fswiki/CorticalParcellation}
#'
#' \itemize{
#'  Frontal lobe:
#'  \item{Label 1002:}{left caudal anterior cingulate}
...
#' }
#'
#' See \code{desikanKillianyTourvilleLabeling} for preprocessing information.
#'
#' @param t1 raw or preprocessed 3-D T1-weighted brain image.
#' @param dkt dkt label image.  If NULL (default), \code{desikanKillianyTourvilleLabeling}
#' is called.
#' @param doPreprocessing perform preprocessing.  See description above.
#' @param antsxnetCacheDirectory destination directory for storing the downloaded
#' template and model weights.  Since these can be resused, if
...
#'
#' image <- antsImageRead( "t1.nii.gz" )
#' results <- dktBasedLobarParcellation( image )
#' }
#' @export
dktBasedLobarParcellation <- function( t1, dkt = NULL, doPreprocessing = TRUE,
  antsxnetCacheDirectory = NULL, verbose = FALSE )
{
  ...
  frontalLabels <- c( 1002, 1003, 1012, 1014, 1018:1020, 1026:1028,
                      2002, 2003, 2012, 2014, 2018:2020, 2026:2028 )
  parietalLabels <- c( 1008, 1010, 1017, 1022:1025, 1029, 1031,
                       2008, 2010, 2018, 2022:2025, 2029, 2031 )
  ...
  for( i in seq.int( length( lobarLabels ) ) )
    ...
    for( j in seq.int( length( lobarLabels[[i]] ) ) )
      {
      dkt[dkt == lobarLabels[[i]][j]] <- i
      }
    }
  dkt[dkt > length( lobarLabels )] <- 0

  atropos <- deepAtropos( t1, doPreprocessing = doPreprocessing,
    antsxnetCacheDirectory = antsxnetCacheDirectory, verbose = verbose )
  brainMask <- antsImageClone( atropos$segmentationImage )
  brainMask[brainMask == 1 | brainMask == 5 | brainMask == 6] <- 0
  brainMask <- thresholdImage( brainMask, 0, 0, 0, 1 )

  lobarParcellation <- iMath( brainMask, "PropagateLabelsThroughMask", brainMask * dkt )

  lobarParcellation[atropos$segmentationImage == 5] <- 5
  lobarParcellation[atropos$segmentationImage == 6] <- 6

  return( lobarParcellation )
}
